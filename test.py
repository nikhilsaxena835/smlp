...
class Ui_MainWindow(object):
    ...
    def insert_clicked(self):
        ...
        self.rightend = self.sketch(self.root, 0, self.rightend)[0]
    ...
    def sketch(self, root, h, y):
        ...
        lnr = self.sketch(root.left, h-3*R, y )
        if root.left:
            self.scene.addLine(lnr[0]+2*R, self.v(h), lnr[1], h-2*R)

        nr = self.sketch(root.right, h-3*R, lnr[0]+2*R)
        if root.right:
            self.scene.addLine(lnr[0] + 2 * R, self.v(h), nr[1], h - 2 * R)


        print("Inserting", root.key)
        label = self.createLabel(str(root.key), "green")
        point = self.scene.addWidget(label)
        point.setPos(self.v(lnr[0]+2*R), self.v(h))
        return [nr[0], lnr[1] + 2 * R]
